...
# Read in eBird data
#dat_br <- readRDS("Data/eBird/RDS/dat_br_x")
#dat_md <- readRDS("Data/eBird/RDS/dat_md_x")
#dat_pb <- readRDS("Data/eBird/RDS/dat_pb_x")
...
#all_points_x <- bind_rows(br_sf_filtered, md_sf_filtered, pb_sf_filtered)
#saveRDS(all_points_x, "Data/eBird/all_points_x.rds")

### Read in RDS for the ### out script
all_points_x <- readRDS("Data/eBird/all_points_x.RDS")

## Replacing empty Park_Addre with Park_Name as to get rid of NAs
all_points <- all_points_x %>%
...
length(unique(all_points$Park_Addre)) #500
...
length(unique(park_counts$Park_Addre)) #82
...
length(unique(park_counts_x$Park_Addre)) #75

### save intermediate script to run analysis for supplementary figure with species richness
#saveRDS(park_counts_x, "Data/Intermediate_Data/park_counts_x.rds")

### see species richness per park
species_richness_df <- park_counts_x %>%
  filter(Park_Addre %in% valid_ids_season$Park_Addre) %>%
  group_by(Park_Addre) %>%
...
final_shapefile_clean_x <- st_as_sf(park_counts_x, 
                                  geometry = st_geometry(park_counts), 
                                  crs = st_crs(park_counts))

# Save as RDS
#saveRDS(final_shapefile_clean_x, "Data/Intermediate_Data/final_shapefile_clean_x.RDS")
...
total_richness_season <- final_shapefile_clean_x %>%
...
total_richness_migratory_status_season <- final_shapefile_clean_x %>%
  st_set_geometry(NULL) %>%
  left_join(avonet, by = c("SCIENTIFIC.NAME" = "Species1")) %>%
  dplyr::select(
...
park_level_data <- final_shapefile_clean_x %>%
  st_set_geometry(NULL) %>%
...
# combine them all
final_data_for_analysis_x <- total_richness_season %>%
...
# Save as shapefile
saveRDS(final_data_for_analysis_x, "Data/AVONET/final_data_for_analysis_x.RDS")

## Now calculate richness by migration status
# Combine AVONET and manual species, keep scientific names
total_richness_migratory_status_scientific_name <- final_shapefile_clean_x %>%
  st_set_geometry(NULL) %>%
  left_join(avonet, by = c("SCIENTIFIC.NAME" = "Species1")) %>%
